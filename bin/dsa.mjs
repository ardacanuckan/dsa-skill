...
function usage() {
  return `Usage:
  dsa render <type> <input.json> [output.html] [--quality standard|showcase] [--repo-root path]
  dsa deliver <type> <input.json> [output.html] [--json] [--open] [--quality standard|showcase] [--repo-root path]
  dsa preview <type> <input.json> [output.html] [--no-open] [--quality standard|showcase] [--repo-root path]
  dsa validate <type> <input.json> [--json] [--layout-json] [--quality standard|showcase] [--repo-root path]
  dsa inspect <type> <input.json>
  dsa migrate workflow <old.json> <new.json> --to-schema 2 [--json]
  dsa check <output.html>
  dsa visual-check <output.html> [--json]
  dsa examples
  dsa doctor

Types: architecture, workflow, sequence, dataflow, lifecycle
--repo-root is architecture only.
`;
}
...
function rendererEnv(quality, repoRoot, diagnosticJson = false) {
  return {
    ...(quality ? { DSA_QUALITY_PROFILE: quality } : {}),
    ...(repoRoot ? { DSA_REPO_ROOT: repoRoot } : {}),
    ...(diagnosticJson ? { DSA_DIAGNOSTIC_FORMAT: 'json' } : {}),
  };
}
...
function sourceEvidenceFromArtifact(artifact) {
  ...
  const match = html.match(/<script id="dsa-source-evidence-data" type="application\/json">([\s\S]*?)<\/script>/);
...
async function commandDeliver(args) {
  ...
  try {
    ...
    reportDeliveryFailure({
      ...
      error: error.message,
      diagnostics: error.dsaDiagnostics || [diagnostic({
  ...
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.dsa-delivery-'));
  } catch (error) {
...
async function commandVisualCheck(args) {
  ...
  try {
    ...
    if (json) {
      ...
      console.error('nobody has looked at the page yet');
    }
    process.exitCode = 1;
    return;
  }
  ...
  if (json) {
    ...
    console.log(`browser check ${result.receipt.status}: ${result.receipt.artifact.path}`);
    console.log(`containment ${result.receipt.containment.status}; captures ${result.receipt.captures.status}; nobody has looked at the page yet`);
...
async function commandDoctor() {
  ...
  if (nodeFailed === 0 && missingFiles === 0 && invalidRuntime === 0) {
    console.log('\nEverything the renderers need is here.');
    return;
  }
...
function migrationPathDiagnostics(error, sourcePath, destinationPath) {
  if (Array.isArray(error?.dsaDiagnostics) && error.dsaDiagnostics.length) {
    return error.dsaDiagnostics.map((entry) => ({
      ...entry,
...
async function commandMigrate(args) {
  ...
  if (
    ...
    fail('Usage: dsa migrate workflow <old.json> <new.json> --to-schema 2 [--json]');
  }
  ...
  try {
    ...
    migration = {
      ...
      migrationDiagnostics: [diagnostic({
        ...
        evidence: { reason: error.message },
        supportedFixes: ['report the source workflow and this diagnostic to the DSA maintainers'],
      })],
    };
  }
  ...
  try {
    ...
    stagingDirectory = fs.mkdtempSync(path.join(destinationDirectory, '.dsa-migration-'));
  } catch (error) {
    reportMigrationFailure({
      ...migration,
      migrationDiagnostics: [...migration.migrationDiagnostics, diagnostic({
  ...
  try {
    ...
    const migrationDiagnostics = Array.isArray(error?.dsaDiagnostics)
...
function commandValidate(args) {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dsa-validate-'));
