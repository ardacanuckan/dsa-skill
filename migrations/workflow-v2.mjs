...
function schemaDiagnostics(workflow) {
  try {
    ...
    if (Array.isArray(error?.dsaDiagnostics)) {
      return error.dsaDiagnostics.map((entry) => ({ ...entry }));
    }
    throw error;
  }
}
  ...
  try {
    ...
    return result({
      ...
      migrationDiagnostics: [diagnostic({
        ...
        supportedFixes: ['report the workflow and compiler receipts to the dsa maintainers'],
      })],
      oldRequiredViewBox,
      newRequiredViewBox: requiredViewBoxFrom(planned),
    });
