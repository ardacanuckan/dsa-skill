...
Omit `meta.legend` for the `auto` default: it lists only the semantic kinds
...
Write everything in the language the user asked for, or the conversation's language when
they did not say. The viewer's own controls are English and `meta.locale` accepts only
`"en"`, so a diagram in another language will have English chrome — say so when it
happens rather than leaving the user to notice.

Keep exact product names, code identifiers, commands, protocols, API paths, and
environment names intact. Those may stay English inside localized copy, but the prose
around them follows the chosen language. Default legend labels are renderer-owned
English; write a `meta.legend.entries.*.label` override when the diagram needs different
domain wording, in the primary language.
...
authored facts, or report the diagnostics as they are.
...
`render`, `validate`, `deliver`, and `preview`; workflow, sequence, dataflow, and
lifecycle reject it. Never infer runtime causality from file
