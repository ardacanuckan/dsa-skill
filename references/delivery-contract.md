...
```bash
node bin/dsa.mjs visual-check <output.html> --json
```
...
```bash
node bin/dsa.mjs preview <type> <input>.json <output>.html --quality showcase
```
...
Opening the file, preview status, and PNG/SVG export are not validation claims.
