...
const LEGEND_CATALOG = [
  ...
  'messagebus',
  'external',
].map((kind) => ({ kind, label: message(`legend.architecture.${kind}`) }));
...
function componentContext(component) {
  ...
  return scopes.length ? scopes.join(' › ') : message('node.context.architecture');
}
...
function renderComponent(c) {
  ...
  const tag = c.tag
    ...
    : '';
  const labelFontSize = fittedNodeFontSize(c.label, c.width, 11, 8);
...
${renderDefinitions({ seed: sketchSeed(arch.meta.title), width: viewBox[0], height: viewBox[1] })}
