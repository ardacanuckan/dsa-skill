...
Render `diagram_type: "dataflow"` JSON files into the standard DSA HTML
template.

```bash
node dsa/renderers/dataflow/render-dataflow.mjs input.dataflow.json output.html
```

The renderer validates input against `dsa/schemas/dataflow.schema.json`
...
`dsa/examples/product-analytics.dataflow.json`.

The schema lives at:

```text
dsa/schemas/dataflow.schema.json
```
...
visual-only because DSA has no compiled edge-kind facts in this slice. A
