...
function renderNode(node) {
  ...
  const context = stage
    ...
    : message('node.context.dataflow');
  const labelFontSize = fittedNodeFontSize(node.label, node.width, 10, 8);
...
const LEGEND_CATALOG = [
  ...
  ...entry,
  label: message(`legend.dataflow.${entry.kind}`),
}));
...
${renderDefinitions({ seed: sketchSeed(dataflow.meta.title), width: viewBox[0], height: viewBox[1] })}
