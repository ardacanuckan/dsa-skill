...
Render `diagram_type: "lifecycle"` JSON files into the standard DSA HTML
template.

```bash
node dsa/renderers/lifecycle/render-lifecycle.mjs input.lifecycle.json output.html
```

The renderer validates input against `dsa/schemas/lifecycle.schema.json`
...
`dsa/examples/agent-run.lifecycle.json`.

The schema lives at:

```text
dsa/schemas/lifecycle.schema.json
```
