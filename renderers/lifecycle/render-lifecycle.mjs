...
function renderState(state) {
  ...
  const step = state.step
    ...
    : '';
  const labelFontSize = fittedNodeFontSize(state.label, state.width, 10, 8);
  const passport = {
    kind: state.type,
    sublabel: state.sublabel,
    tag: state.tag,
    context: laneLabels.get(state.lane) || message('node.context.lifecycle'),
  };
...
const LEGEND_CATALOG = [
  ...
  'neutral',
  'external',
].map((kind) => ({ kind, label: message(`legend.lifecycle.${kind}`) }));
...
${renderDefinitions({ seed: sketchSeed(lifecycle.meta.title), width: viewBox[0], height: viewBox[1] })}
