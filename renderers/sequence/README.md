...
Render `diagram_type: "sequence"` JSON files into the standard DSA HTML
template.

```bash
node dsa/renderers/sequence/render-sequence.mjs input.sequence.json output.html
```

The renderer validates input against `dsa/schemas/sequence.schema.json`
...
`dsa/examples/cache-miss-request.sequence.json`.

The schema lives at:

```text
dsa/schemas/sequence.schema.json
```
