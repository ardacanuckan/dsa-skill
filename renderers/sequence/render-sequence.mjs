...
function renderParticipant(participant) {
  ...
  const sub = hasSub
    ...
    : '';
  const labelFontSize = fittedNodeFontSize(participant.label, layout.participantW, 11, 8);
  const passport = {
    kind: participant.type,
    sublabel: participant.sublabel,
    context: message('node.context.sequence'),
  };
...
const LEGEND_CATALOG = [
  ...
  swatchWidth: 34,
  swatchGap: 9,
  label: message(`legend.sequence.${entry.kind}`),
}));
...
${renderDefinitions({ seed: sketchSeed(sequence.meta.title), width: viewBox[0], height: viewBox[1] })}
