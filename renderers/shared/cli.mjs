...
import { text as message } from './strings.mjs';
...
export function loadDiagram({ rendererDir, diagramType, defaultExample, argv = process.argv }) {
  ...
  const sourceEvidence = verifyRepositoryEvidence(diagramType, diagram, process.env.DSA_REPO_ROOT);
...
export function svgRootAttrs(meta) {
  ...
  const engineeringProfile = meta.engineering_profile
    ...
    : '';
  const requestedProfile = process.env.DSA_QUALITY_PROFILE || meta.quality_profile;
  ...
  return `role="img" lang="en" aria-labelledby="dsa-diagram-title dsa-diagram-description"${animation}${engineeringProfile} data-quality-profile="${esc(qualityProfile)}"${advisory}`;
}

// Keep the accessible name inside the SVG so it survives standalone SVG
// export and embedding. The fixed IDs are deterministic because a DSA
// artifact contains exactly one primary diagram SVG.
export function svgAccessibleText(meta, kind) {
  const description = meta.subtitle || message(`diagram.description.${kind}`);
  return `        <title id="dsa-diagram-title">${esc(meta.title)}</title>\n        <desc id="dsa-diagram-description">${esc(description)}</desc>`;
}
...
export function focusNodeAttrs(id, label, metadata = {}) {
  ...
  const detail = [metadata.sublabel, metadata.context]
    ...
    .join(', ');
  const aria = detail
    ? message('node.focus.detail', { label, detail })
    : message('node.focus', { label });
...
// canonical SVG is embedded inline outside the full DSA viewer.
