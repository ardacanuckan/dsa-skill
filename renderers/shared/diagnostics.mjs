...
const DIAGNOSTIC_MODE = process.env.DSA_DIAGNOSTIC_FORMAT === 'json';
const recorded = [];
const recordedMessages = new Set();
const boundaryKey = Symbol.for('dsa.renderer-diagnostic-boundary');
...
function normalizedDiagnostic(diagnostic) {
  const message = String(diagnostic?.message || 'DSA could not classify this failure.').trim();
  return {
...
export function throwDiagnosticError(message, diagnostics) {
  ...
  error.dsaDiagnostics = (diagnostics || []).map(normalizedDiagnostic);
  throw error;
}
...
function rendererFailure(error) {
  const attached = Array.isArray(error?.dsaDiagnostics)
    ? error.dsaDiagnostics.map(normalizedDiagnostic)
    : [];
