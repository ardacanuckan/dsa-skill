...
export function renderLegend({ entries, layout, renderSwatch }) {
  ...
  const parts = [
    `        <g${rootAttributes}>`,
    `          <rect data-legend-plate="" x="${layout.x - 3}" y="${measured.titleY - 10}" width="${titleWidth}" height="14" rx="6" class="c-legend-plate"/>`,
    `          <text x="${layout.x + 1}" y="${measured.titleY}" class="t-primary" font-size="9" font-weight="700">${esc(message('legend.title'))}</text>`,
  ];
  ...
  for (const entry of measured.entries) {
    ...
    parts.push(`            <rect data-legend-plate="" x="${entry.x - 3}" y="${entry.baseline - 10}" width="${entry.width + 6}" height="14" rx="6" class="c-legend-plate"/>`);
