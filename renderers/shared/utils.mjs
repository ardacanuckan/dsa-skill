import { escapeHtml as esc, text as message } from './strings.mjs';

export { esc };

// One deterministic seed per diagram keeps the pen wobble stable across
// re-renders, so a re-delivered artifact is byte-comparable.
export function sketchSeed(title) {
  let hash = 0;
  for (const character of String(title ?? '')) hash = (hash * 31 + character.codePointAt(0)) % 9973;
  return hash;
}

export function renderDefinitions({ seed = 7, width = 1000, height = 680 } = {}) {
  // userSpaceOnUse, not objectBoundingBox: an axis-aligned arrow has a
  // zero-height bounding box, and SVG disables the filter *and drops the
  // element* in that case. One shared user-space region also gives every
  // stroke the same sheet of paper to wobble against.
  const pad = 48;
  return `        <defs>
          <filter id="pen" filterUnits="userSpaceOnUse" x="${-pad}" y="${-pad}" width="${width + pad * 2}" height="${height + pad * 2}" color-interpolation-filters="sRGB">
            <feTurbulence type="fractalNoise" baseFrequency="0.02" numOctaves="2" seed="${seed}" result="wobble"/>
            <feDisplacementMap in="SourceGraphic" in2="wobble" scale="2.9" xChannelSelector="R" yChannelSelector="G"/>
          </filter>
          <marker id="arrowhead" markerWidth="9" markerHeight="7" refX="8" refY="3.5" orient="auto">
            <polygon points="0 0, 9 3.5, 0 7" class="m-default" />
          </marker>
          <marker id="arrowhead-emphasis" markerWidth="9" markerHeight="7" refX="8" refY="3.5" orient="auto">
            <polygon points="0 0, 9 3.5, 0 7" class="m-emphasis" />
          </marker>
          <marker id="arrowhead-security" markerWidth="9" markerHeight="7" refX="8" refY="3.5" orient="auto">
            <polygon points="0 0, 9 3.5, 0 7" class="m-security" />
          </marker>
          <marker id="arrowhead-dashed" markerWidth="9" markerHeight="7" refX="8" refY="3.5" orient="auto">
            <polygon points="0 0, 9 3.5, 0 7" class="m-dashed" />
          </marker>
          <pattern id="grid" width="24" height="24" patternUnits="userSpaceOnUse">
            <circle cx="1.2" cy="1.2" r="1.05" class="c-grid"/>
          </pattern>
        </defs>`;
}
...
// The three conclusion cards are sticky notes. `dot` picks the note colour.
export function renderCards(cards) {
  const list = Array.isArray(cards) ? cards : [];
  return `    <!-- DSA:CARDS_SLOT_START -->
    <div class="notes">
${list.map((card) => `      <div class="note" data-note-color="${esc(card.dot)}">
...
const SVG_SLOT_RE = /[ \t]*<!-- DSA:SVG_SLOT_START -->[\s\S]*?<!-- DSA:SVG_SLOT_END -->/;
const CARDS_SLOT_RE = /[ \t]*<!-- DSA:CARDS_SLOT_START -->[\s\S]*?<!-- DSA:CARDS_SLOT_END -->/;
const SUBTITLE_SLOT_RE = /^([ \t]*)<p class="subtitle">\[Subtitle description\]<\/p>[ \t]*(\r?\n)?/m;
const VIEWS_PLACEHOLDER = '<!-- DSA:VIEWS_DATA -->';
const SOURCE_EVIDENCE_PLACEHOLDER = '<!-- DSA:SOURCE_EVIDENCE_DATA -->';
...
export function applyTemplate(template, {
  ...
  if (!SVG_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing DSA:SVG_SLOT sentinel');
  }
  if (!CARDS_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing DSA:CARDS_SLOT sentinel');
  }
  ...
  // Dropping verified evidence silently would be misleading, so the slot is
  // required only on the opt-in evidence path.
  ...
  // Function replacers: a literal `$&`, `$'`, `` $` `` or `$$` in titles,
  // labels, or rendered SVG must not act as a replacement pattern.
  const viewsJson = serializeScriptJson(guidedViews);
  ...
  const renderedSubtitle = typeof subtitle === 'string' && subtitle.trim()
    ...
    : '';
  return template
    .replace(TEMPLATE_PLACEHOLDERS[0], () => `<title>${esc(message('page.title', { title }))}</title>`)
    ...
    .replace(VIEWS_PLACEHOLDER, () => `<script id="dsa-views-data" type="application/json">${viewsJson}</script>`)
    .replace(SOURCE_EVIDENCE_PLACEHOLDER, () => (sourceEvidence
      ? `<script id="dsa-source-evidence-data" type="application/json">${sourceEvidenceJson}</script>`
      : ''));
}
