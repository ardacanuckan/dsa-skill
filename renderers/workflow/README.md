...
Render `diagram_type: "workflow"` JSON files into the standard DSA HTML
template.

```bash
node dsa/renderers/workflow/render-workflow.mjs input.workflow.json output.html
```

The renderer validates input against `dsa/schemas/workflow.schema.json`
...
```bash
node dsa/scripts/check-render-output.mjs output.html
```
...
`dsa/examples/agent-tool-call.workflow.json`; its `schema_version` selects
the applicable contract.

The schema lives at:

```text
dsa/schemas/workflow.schema.json
```
...
```bash
node dsa/bin/dsa.mjs migrate workflow old.json new.json --to-schema 2 --json
```
...
```bash
node dsa/bin/dsa.mjs validate workflow input.workflow.json --layout-json
```
...
Set `meta.quality_profile` to `showcase` to turn the composition checks into errors. Unrelated proper
