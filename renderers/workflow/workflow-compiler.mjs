import { esc, renderDefinitions, sketchSeed, renderSemanticSigil, textUnits } from '../shared/utils.mjs';
...
import { text as message } from '../shared/strings.mjs';
import {
...
function compileWorkflowInternal({
  ...
  try {
    ...
    inputDiagnostics = Array.isArray(error?.dsaDiagnostics)
      ? error.dsaDiagnostics.map((diagnostic) => ({
          ...diagnostic,
          supportedFixes: [],
        }))
      : [{
...
const LEGEND_CATALOG = [
  ...
  'cloud',
  'external',
].map((kind) => ({ kind, label: message(`legend.workflow.${kind}`) }));
...
function readableAutomaticRoute(edge, from, to, primarySides, primaryPorts) {
  ...
  if (authoredSideFields.length) {
    ...
    const attemptedEvidence = firstFailure?.dsaDiagnostics?.[0]?.evidence || {};
...
${renderDefinitions({ seed: sketchSeed(workflow.meta.title), width: viewBox[0], height: viewBox[1] })}
...
${renderLegend()}
  ...
  try {
    ...
    if (!Array.isArray(error?.dsaDiagnostics)) throw error;
    const diagnostics = error.dsaDiagnostics.map((diagnostic) => ({ ...diagnostic }));
