...
It also accepts `locale`, which takes only `"en"`. The viewer's own controls,
default legend labels and accessibility copy are English; the field translates
nothing. Omitting it is the same as writing it.
...
explicit authored viewBox without overlap, DSA omits the complete legend
...
can be produced explicitly with `dsa migrate workflow ... --to-schema 2`.
...
- `componentType` — `frontend`, `backend`, `database`, `cloud`, `security`,
  `messagebus`, `external`
- `locale` — the renderer locale, `en` only
...
strictly DSA judges composition. `engineering_profile` is an optional
