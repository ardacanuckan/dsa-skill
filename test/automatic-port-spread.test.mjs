...
function render(mode, doc) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dsa-port-spread-'));
...
test('the skill and the authoring contract describe automatic port spread', () => {
  const skill = fs.readFileSync(path.join(skillRoot, 'SKILL.md'), 'utf8');
  assert.match(skill, /Shared automatic endpoints spread themselves/);
  assert.match(skill, /skips\s+any relationship you routed by hand/);
