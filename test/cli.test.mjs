...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dsa-cli-'));
const cli = path.join(skillRoot, 'bin/dsa.mjs');
...
fs.writeFileSync(process.env.DSA_TEST_OPEN_LOG, JSON.stringify({
...
test('cli: help lists commands and diagram types', () => {
  ...
  assert.match(result.stdout, /dsa render <type>/);
  assert.match(result.stdout, /dsa deliver <type>/);
  assert.match(result.stdout, /dsa preview <type>/);
  assert.match(result.stdout, /dsa visual-check <output\.html>/);
  assert.match(result.stdout, /--open/);
  assert.match(result.stdout, /--repo-root is architecture only\./);
  assert.match(result.stdout, /dsa doctor/);
...
test('cli: doctor reports a complete installation is ready', () => {
  ...
  assert.match(result.stdout, /\[ok\] Authoring references/);
  ...
  assert.match(result.stdout, /Everything the renderers need is here\./);
});
...
test('cli: doctor identifies an incomplete installation', () => {
  ...
  fs.copyFileSync(cli, path.join(incompleteBin, 'dsa.mjs'));

  const result = spawnSync(process.execPath, [path.join(incompleteBin, 'dsa.mjs'), 'doctor'], {
    cwd: incompleteRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /Not ready: \d+ required files? missing\./);
});
...
test('cli: doctor rejects a corrupt standalone validator', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(corruptRoot, 'bin/dsa.mjs'), 'doctor'], {
    cwd: corruptRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /Not ready: 1 runtime check failed\./);
});
...
test('cli: examples renders from an installed skill', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(installedRoot, 'bin/dsa.mjs'), 'examples'], {
    cwd: installedRoot,
    encoding: 'utf8',
  });
...
test('cli: visual-check returns a skipped receipt with exit 2 when Chrome is unavailable', () => {
  ...
  const result = run(['visual-check', out, '--json'], {
    env: { ...process.env, DSA_CHROME: missingChrome },
  });
...
test('cli: visual-check reports a browser check, never visual approval', () => {
  ...
  assert.equal(result.status, 2, result.stderr);
  assert.match(result.stdout, /browser check skipped:/i);
  assert.match(result.stdout, /nobody has looked at the page yet/i);
  assert.doesNotMatch(result.stdout, /^visual-check skipped:/m);
});

test('cli: visual-check keeps the machine check and the human look apart on input failure', () => {
  const result = run(['visual-check', path.join(tmp, 'missing-browser-evidence.html')]);

  assert.equal(result.status, 1);
  assert.match(result.stderr, /browser check failed:/i);
  assert.match(result.stderr, /nobody has looked at the page yet/i);
...
test('cli: a missing optional opener module preserves verified delivery with a fallback receipt', () => {
  ...
  copyInstalledSkill(installedRoot);
  const installedCli = path.join(installedRoot, 'bin/dsa.mjs');
...
test('cli: preview runs from an installed skill without node_modules and exits cleanly', { timeout: 30000 }, async () => {
  ...
  assert.deepEqual(fs.readdirSync(path.dirname(output)).filter((name) => name.startsWith('.dsa-preview-')), []);
});
...
test('cli: deliver preserves the previous artifact when the final check fails', () => {
  ...
  assert.deepEqual(
    fs.readdirSync(path.dirname(out)).filter((name) => name.includes('.dsa-delivery-')),
    [],
  );
});
...
test('cli: validate JSON exposes only the primary v1 column-capacity diagnostic', () => {
  ...
  const result = run(['validate', 'workflow', input, '--json'], {
    env: { ...process.env, DSA_DIAGNOSTIC_FORMAT: 'json' },
  });
