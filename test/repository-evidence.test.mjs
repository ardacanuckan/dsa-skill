...
function fixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'dsa-evidence-repo-'));
  ...
  git(root, 'init');
  git(root, 'config', 'user.name', 'DSA Tests');
  git(root, 'config', 'user.email', 'dsa@example.test');
...
test('repository evidence is opt-in and never appears in ordinary artifacts', () => {
  const output = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'dsa-no-evidence-')), 'plain.html');
  const input = path.join(skillRoot, 'examples', 'web-app.architecture.json');
  const result = run(['render', 'architecture', input, output]);
  assert.equal(result.status, 0, result.stderr);
  const html = fs.readFileSync(output, 'utf8');
  assert.doesNotMatch(html, /id="dsa-source-evidence-data"/);
  const svg = html.match(/<svg\b[\s\S]*?<\/svg>/)?.[0] || '';
