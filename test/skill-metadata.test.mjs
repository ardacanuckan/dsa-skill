...
test('main skill stays a bounded authoring router with progressive references', () => {
  ...
  assert.ok(lines.length <= 140, `SKILL.md is ${lines.length} lines; keep the entrypoint at 140 or fewer`);
...
test('the skill still says what it dropped and what it kept', () => {
  assert.match(skill, /Omit `meta\.subtitle`/);
  assert.doesNotMatch(skill, /visual_preset|share card|brand mark|WebM|check-update/i);
