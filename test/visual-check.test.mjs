...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dsa-visual-check-'));
...
test('visual-check records four containment viewports and four endpoint theme captures', async () => {
  ...
  assert.match(contactSheet, /Browser check/);
  assert.match(contactSheet, /nobody has looked at the page yet/);
...
test('visual-check returns 2 with a truthful skipped receipt when Chrome is unavailable', async () => {
  ...
  assert.ok(result.receipt.diagnostics[0]?.supportedFixes.some((fix) => fix.includes('DSA_CHROME')));
  assert.equal(fs.existsSync(sidecarPaths(input).receipt), true);
});
