...
test('an explicit standard profile overrides an ambient showcase profile', () => {
  const document = nestedOutsideRightCorridorWorkflow();
  document.meta.quality_profile = 'showcase';
  const previousProfile = process.env.DSA_QUALITY_PROFILE;

  try {
    process.env.DSA_QUALITY_PROFILE = 'showcase';
    const result = compileWorkflow({ workflow: document, qualityProfile: 'standard' });
    assert.equal(
      result.ok,
      true,
    ...
    if (previousProfile === undefined) delete process.env.DSA_QUALITY_PROFILE;
    else process.env.DSA_QUALITY_PROFILE = previousProfile;
  }
});
